...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V>
        ...
        private HashEntry<K,V> scanAndLockForPut(K key, int hash, V value) {
            ...
            while (!tryLock()) {
                ...
                if (retries < 0) {
                    ...
                    // 如果hash对应的index上，有元素，但是key不相同。说明已经发生了hash冲突。需要遍历到链表的结尾进行插入。
                        e = e.next;
                }
                // 如果失败的次数大于MAX_SCAN_RETRIES，则需要直接进行加锁了。同时退出。
                // MAX_SCAN_RETRIES居然有64这么大
                else if (++retries > MAX_SCAN_RETRIES) {
                    lock();
                    break;
                }
                // 如果entryForHash的值变了。则重新进行尝试。
                // (retries & 1) == 0 ? 尝试的偶数词才进行检查？
    ...
    public int size() {
        ...
        try {
            // 如果本次计算的sum和上一次计算的sum一样。那么就退出。
            // 如果本次计算的sum和上一次计算的sum不一样。那么继续来计算sum。
            for (;;) {
                // 如果retries（尝试）计算sum的次数小于RETRIES_BEFORE_LOCK, 那么不锁，再计算一次。
                // 如果retries（尝试）计算sum的次数等于RETRIES_BEFORE_LOCK的时候，就会先加锁。然后再计算size()
                // 也就是说最开始是默认没有锁的，也估计没有并发。但是计算了一次发现不对。有并发。再试一次。如果还是有并发。再加锁来进行计算。
                // 这样能够减少加锁的次数呗。
            ...
            // 如果加过锁了。那么就需要对所有的segments进行解锁。
            // 看来对所有的segements加锁是一个很重的逻辑。不要轻易加锁。
            if (retries > RETRIES_BEFORE_LOCK) {
                for (int j = 0; j < segments.length; ++j)
     ...
     */
    @SuppressWarnings("unchecked")
    // get和containsKey的逻辑很相似。只不过一个返回的是Object和null；一个返回的是true和false
    ...
    public boolean containsValue(Object value) {
        // Same idea as size()
        // 首先是不加锁的来进行查找。如果在查找的过程中。找到了并且没有被改变。那么直接返回就好了。
        // 如果在找的过程中，无论找到还是没有找到，都被并发的线程进行更改过。那么需要重新来找。
        // 如果这样重新来找了两次，还是被并发更改，那么就会尝试去对所有的segment来进行加锁。
        if (value == null)
    ...
    public V put(K key, V value) {
        Segment<K,V> s;
        // concurrentHashMap里面不许允许存放value == null的
        if (value == null)
            throw new NullPointerException();
        int hash = hash(key);
        ...
        if ((s = (Segment<K,V>)UNSAFE.getObject          // nonvolatile; recheck
            ...
            s = ensureSegment(j);
        // 首先找到应该存放的那个segment，然后再调用segment的put            
     ...
     *
     * 删除map中的key,如果map中不含有这个key,那么该方法不会进行任何操作。
     *
